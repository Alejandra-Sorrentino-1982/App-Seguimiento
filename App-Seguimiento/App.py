...
#Python 3.12
#Framework Flask; MySql; Pandas; Bytes10
...
#Borrar una carpeta en Git
#$ git rm -r NOMBRE DE LA CARPETA

#El commit
#$ git commit -m "Elimino mi carpeta"

#El push para que el cambio se haga en Github
#$ git push
...
def login():
    ...
    if request.method == 'POST':
        ...
        if login:  # Verificamos que se haya encontrado un usuario
            if password == login[2]:  # Suponiendo que 'password' es el tercer campo (índice 2)
...
#datos admin - manejo de los datos de los usuarios administradores/editores
@app.route('/usuarios')
def usuarios():
    cur = mysql.connection.cursor()
    cur.execute('SELECT * FROM login') #pido la tabla desde sql
    tablasql = cur.fetchall() #trae toda la tabla
    return render_template('usuarios.html', usuarios = tablasql) #render template: esto tiene que retornar la página index, la carpeta template ya la tiene preconfigurada
...
def Agregar4():
  if request.method == 'POST':
     ...
     cur = mysql.connection.cursor()
     cur.execute('INSERT INTO login (username, password, tipo) VALUES (%s, %s, %s)', (username, password, tipo)) 
     mysql.connection.commit()
...
def actualizar_tabla4(id):
 if request.method == 'POST':
      ...
      tipo = request.form['tipo']
      cur = mysql.connection.cursor()     
      cur.execute('UPDATE `login` SET `username`= %s,`password`= %s,`tipo`= %s WHERE id= %s', (username, password, tipo, id,))
      mysql.connection.commit() 
